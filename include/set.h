...
/*global interface*/
extern int       set_add(SET_S *set, int bit);
...
extern void      set_op(int op, SET_S *dst, SET_S *src);
extern int       set_test(SET_S *set1, SET_S *set2);
...
extern void      set_dump(SET_S *set);
...
#define SET_UNION(d, s)         set_op(_UNION_OP, d, s)
#define SET_INTERSECT(d, s)     set_op(_INTERSECT_OP, d, s)
#define SET_DIFFERENCE(d, s)    set_op(_DIFFERENCE_OP, d, s)
#define SET_ASSIGN(d, s)        set_op(_ASSIGN_OP, d, s)
...
#define SET_IS_DISJOINT(s1, s2)    (set_test(s1, s2) == _SET_DISJ)
#define SET_IS_INTERSECT(s1, s2)   (set_test(s1, s2) == _SET_INTER)
...
#define SET_REMOVE(s, x)           (((x) >= (s)->nbits) ? 0 : _SET_GBIT(s, x , &= ~))
#define SET_ADD(s, x)              (((x) >= (s)->nbits) ? set_add(s, x) : _SET_GBIT(s, x, |=))
#define SET_MEMBER(s, x)           (((x) >= (s)->nbits) ? 0 : _SET_GBIT(s, x, &))
