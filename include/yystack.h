...
#define yypush(stack, x)               (yystk_full(stack) ? yystk_err(1) : yypush_(stack, x))

#define yypop(stack)                   (yystk_empty(stack) ? yystk_err(0) : yypop_(stack))

#define yypopn_(stack, amt)            ((yyp_##stack += amt)[-amt])
#define yypopn(stack, amt)             (((yystk_ele(stack)) < amt) ? yystk_err(0) : yypopn_(stack,amt))

#define yystk_err(o)                 ((o) ? com_ferr("Stack overflow.\n") : com_ferr("Stack underflow.\n"))
